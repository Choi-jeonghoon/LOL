...
export class ProcessService {
  ...
  extractMatchData(data: {
    ...
    const extractedData = {
      ...
      gameStartTimestamp: convertTimestampToDate(
        Number(data.info.gameStartTimestamp),
      ), // 시작 시간
      gameEndTimestamp: convertTimestampToDate(
        Number(data.info.gameEndTimestamp),
      ), // 종료 시간
      participants: [],
      teams: [],
    };
    ...
    extractedData.participants = data.info.participants.map((participant) => ({
      ...
      teamPosition:
        participant.teamPosition === 'UTILITY'
          ? 'SUPPORT'
          : participant.teamPosition, //선택된 라인
    ...
    extractedData.teams = data.info.teams.map((team) => ({
      ...
      bans: team.bans
        .map((ban: { championId: number }) => ban.championId)
        .filter((championId: number) => championId !== -1), //각팀의 챔피언 금지 선택한 목록
    }));
